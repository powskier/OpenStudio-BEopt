...
#start the measure
class ResidentialHotTubHeaterGas < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    units.each do |unit|
        ...
        if hth_ann_g > 0
            ...
            if sch.nil?
                # Create schedule
                sch = MonthWeekdayWeekendSchedule.new(model, runner, Constants.ObjectNameHotTubHeater(Constants.FuelTypeGas) + " schedule", weekday_sch, weekend_sch, monthly_sch)
    ...
    if msgs.size > 1
        ...
        runner.registerFinalCondition("The building has been assigned hot tub heaters totaling #{tot_hth_ann_g.round} therms annual energy consumption across #{units.size} units.")
    elsif msgs.size == 1
...
ResidentialHotTubHeaterGas.new.registerWithApplication