...
class ResidentialHotWaterHeaterTanklessElectric < OpenStudio::Measure::ModelMeasure
    ...
    def run(model, runner, user_arguments)
        ...
        units.each do |unit|
            ...
            if loop.supplyOutletNode.setpointManagers.empty?
                new_manager = Waterheater.create_new_schedule_manager(t_set, model,"tankless")
    ...
    def list_water_heaters(model, runner)
        ...
        for heater in existing_heaters do
            ...
            msg = "Water heater '#{heatername}' added to plant loop '#{loopname}', with a capacity of #{capacity.round(1)} kW and a burner efficiency of  #{te.round(2)}, in thermal zone '#{heater.ambientTemperatureThermalZone.get.name}'"
