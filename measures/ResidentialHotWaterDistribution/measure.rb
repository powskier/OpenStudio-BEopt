...
class ResidentialHotWaterDistribution < OpenStudio::Measure::ModelMeasure
	...
	def run(model, runner, user_arguments)
        ...
        units.each do |unit|
            ...
            weeks = 53
            sch_sh = HotWaterSchedule.new(model, runner, "", "", nbeds, sch_unit_index, 0, "Shower", Constants.MixedUseT, File.dirname(__FILE__), weeks, false)
            sch_s = HotWaterSchedule.new(model, runner, "",  "", nbeds, sch_unit_index, 0, "Sink", Constants.MixedUseT, File.dirname(__FILE__), weeks, false)
            sch_b = HotWaterSchedule.new(model, runner, "",  "", nbeds, sch_unit_index, 0, "Bath", Constants.MixedUseT, File.dirname(__FILE__), weeks, false)
