...
class ResidentialHotWaterHeaterTankFuel < OpenStudio::Measure::ModelMeasure
    ...
    def list_water_heaters(model, runner)
        ...
        for heater in existing_heaters do
            ...
            msg = "Water heater '#{heatername}' added to plant loop '#{loopname}', with a capacity of #{capacity.round(1)} kBtu/hr and an actual tank volume of #{volume.round(1)} gal, in thermal zone '#{heater.ambientTemperatureThermalZone.get.name}'"
